...
版权所有 \copyright ~2014 Allen B. Downey.
...
信号处理是我最喜欢的主题之一，对许多科学和工程领域都十分有
用。而且，如果理解了其基本概念，信号处理可以帮我们洞察世间
万物，尤其是可以用听觉感知的事物。

然而，若非你研修过电子或机械工程，你很可能并没有多少机会学
到信号处理。困难在于大多数书（以及使用这些书的课程）都采取
自底而上的方式，从诸如相量（phasor）这样的数学抽象讲起，而
且偏向理论，几乎没有和实际相关的应用。

本书则假定：会编程就可以通过编程技巧学习其它知识，并从中收
获乐趣。

采取基于编程的方式，我可以立即展现最重要的概念。学完第一章，
你就可以分析声音的记录和其它信号，还能生成新的声音。每章都
会引入一项新技术，并把它应用在真实信号上。每步你都将先学会
如何使用一项技术，之后才是其工作原理。

这种方式更加实用，而且（希望你能同意）也更有趣。


\section{本书面向的读者}

本书里的示例代码基于Python。你需要懂得基本的Python代码并熟
悉面向对象。你无需自行设计对象，但至少要会使用对象。

如果你不熟悉Python，也许可以先看我的另一本书，向从未编过程
序的人介绍Python的{\it 《像计算机科学家一样思考Python》}。或
者看看Mark Lutz的{\it 《 Python学习手册 》}，这本更适合有编程
经验的人。

我大量使用了NumPy和SciPy。如果你很熟悉这两个包，非常好。
不管怎样，我都会解释用到的函数和数据结构。

我假设读者掌握基本的数学知识，包括复数。不需要太多微积分，
理解积分和微分的概念即可。我用到一些线性代数的时候会讲解。


\section{使用随书代码}
\label{code}

本书用到的代码和声音样例位于\url{https://github.com/AllenDowney/ThinkDSP}
（中译本：\url{https://github.com/boltomli/ThinkDSP}）。Git版
本控制系统让你能够追踪构成一个项目的文件。在Git控制下的一
组文件的集合称为一个“仓库”。GitHub是一个提供在线Git仓库
存储的服务商，其网络界面十分好用。
\index{repository}
\index{Git}
\index{GitHub}

GitHub为我的仓库提供了几种利用代码的方法：

\begin{itemize}

\item 如果你还没有GitHub帐号，你得先创建一个。然后可以通过
{\sf Fork}按钮在GitHub上建立一个我的仓库的拷贝。分支后，你就
将在GitHub上拥有自己的仓库来追踪代码。你可以克隆这个库，
也就是说把文件拷贝到你的电脑上。
\index{fork}

\item 你也可以直接克隆我的仓库，不需要GitHub帐号。不过你无
法把你的改动写回到GitHub。
\index{clone}

\item 如果你并不想使用Git，还可以通过GitHub页面右侧的按钮
下载包含全部文件的Zip文件。

\end{itemize}

所有代码都能正常工作于Python 2和Python 3。

我开发这本书用的是Continuum Analytics的Anaconda，这是个免费
的Python发行版，包含了运行代码需要的全部包（以及更多）。我
发现Anaconda安装很容易，默认情况下安装到用户目录而非系统目
录，所以不需要管理员权限，而且支持Python 2和Python 3。你可以
下载Anaconda：\url{http://continuum.io/downloads}。
\index{Anaconda}

如果你不想用Anaconda，你需要如下的包：

\begin{itemize}

\item NumPy，基本的数字计算，\url{http://www.numpy.org/};
\index{NumPy}

\item SciPy，科学计算，\url{http://www.scipy.org/};
\index{SciPy}

\item matplotlib，可视化，\url{http://matplotlib.org/}.
\index{matplotlib}

\end{itemize}

尽管这些包都很常用，仍然不是所有Python安装都包含了这些包，
而且有些环境下可能很难单独安装。如果你安装这些包有困难，
我推荐使用Anaconda或者其它包含这些包的Python发行版。
\index{installation}

大多数习题使用Python脚本，也有些使用Jupyter notebook。如果
你以前没用过Jupyter，可以看看文档：\url{http://jupyter.org}。
\index{Jupyter}

有三种使用Jupyter Notebook的方式：

\begin{description}

\item[在你的电脑上运行Jupyter]

如果你安装了Anaconda，你可能已经有了Jupyter。如此从命令行
启动服务器：
...
如果没有安装，在Anaconda里可以通过命令行安装：

\begin{verbatim}
$ conda install jupyter
\end{verbatim}

服务器启动后，将开启默认浏览器或在浏览器窗口中打开新选项卡。

\item[在Binder上运行Jupyter]

Binder是一个在虚拟机中运行Jupyter的服务。假如你访问链接
\url{http://mybinder.org/repo/AllenDowney/ThinkDSP}，
你将看到本书的Jupyter主页，涵盖了辅助数据和脚本。

你可以运行脚本或修改脚本以运行你自己的代码，但虚拟机是临时的。
在空闲大约一小时之后，任何改动将随着虚拟机一同消失。

\item[在nbviewer上查看]

在本书中，我们提到notebook的时候将提供指向nbviewer的链接，
用来查看代码及结果的静态版本。你可以通过这些链接阅读Notebook
并聆听示例，但无法修改或运行代码，也没法使用互动部件。
...
如果你有建议或修订意见，请发信给{\tt downey@allendowney.com}
（中译本独有问题请联系{\tt songl@outlook.com}）。如果我基于你
的反馈做了修改，我将把你列入贡献者列表（除非你要求不要这样
做）。
\index{contributors}

请至少给出错误所在的句子的一部分，这样比较容易搜索。页码和
章节编号也可以，但并不太方便。谢谢！
