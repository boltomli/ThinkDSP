...
{\sf Fork}按钮在GitHub上建立一个我的仓库的拷贝。分支后，你
就将在GitHub上拥有你自己的仓库来追踪代码。你可以克隆这个库，
也就是说把文件拷贝到你的电脑上。
\index{fork}

\item 你也可以克隆我的仓库，不需要GitHub帐号。不过你没法把
你的改动写回到GitHub。
\index{clone}
...
尽管这些都是很常用的包，并非每个Python安装都包含了这些包，
在有些环境下可能很难安装。如果你安装这些包有困难，我推荐
使用Anaconda或者任一其它包含了这些包的Python发行版。
\index{installation}

大多数习题使用了Python脚本，也有些使用IPython notebook。如果
